	...
	<div class="container">
		...
		<table class="table">
			<tr>
				...
				<td colspan="3" width="450px">${boardItem.id}</td>
			</tr>
			<tr>
		...
		<div class="div-button">
			<input type="button" value="목록"
				onclick="location.href='/boardItem/list'"> <input
				type="button" value="수정"
				onclick="location.href='/boardItem/update?id=${boardItem.id}'">
		...
		<div class="align-right">
			<input type="button" value="댓글 달기"
				onclick="location.href='/replyItem/root/insert?id=${boardItem.id}'">
		</div>
		<table class="table-reply">
			<c:forEach var="replyItem" items="${boardItem.newsReplyItems}">
				<c:if test="${replyItem.id eq replyItem.rootReplyItem.id}">
					...
					<tr>
						<td colspan="2">${replyItem.writer} | ${replyItem.date} | <a
							href="/replyItem/sub/insert?id=${replyItem.id}">답글</a> | <a
							href="/replyItem/root/update?id=${replyItem.id}">수정</a> | <a
							href="/replyItem/root/delete?id=${replyItem.id}">삭제</a>
						</td>
					</tr>
					<tr>
					...
					<c:forEach var="subReplyItem" items="${replyItem.subReplyItems}">
						<c:if
							test="${subReplyItem.id ne replyItem.id && subReplyItem.rootReplyItem.id eq replyItem.id}">
							<tr>
								<td width=5%>&nbsp;&nbsp;&nbsp;&nbsp;</td>
								<td>${subReplyItem.writer} | ${subReplyItem.date} | <a
									href="/replyItem/sub/update?id=${subReplyItem.id}">수정</a> | <a
									href="/replyItem/sub/delete?id=${subReplyItem.id}">삭제</a>
								</td>
							</tr>
							<tr>
								<td width=5%>&nbsp;&nbsp;&nbsp;&nbsp;</td>
